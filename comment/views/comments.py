...
class CreateComment(CanCreateMixin, CommentCreateMixin):
    ...
    def form_valid(self, form):
        ...
        if self.comment.parent:
            if self.comment.parent.user.email in [author_email,user_email]:
                parent_email = False
            else:
        ...
        if author_email:
            email = EmailMessage(
                        'دریافت دیدگاه',
                        'برای مقاله {} دیدگاه جدید ثبت شده است برای پاسخ دادن بر روی لینک زیر کلیک کنید\n{}{}.'.format(article,current_site,reverse('article:detail',kwargs={'slug':article.slug})),
                        to=[author_email]
            )
            email.send()
        ...
        if user_email:
            email = EmailMessage(
                        'ارسال دیدگاه',
                        'دیدگاه شما برای مقاله {} ثبت گردید.'.format(article),
                        to=[user_email]
            )
            email.send()
        ...
        if parent_email:
            email = EmailMessage(
                        'دریافت دیدگاه',
                        'برای دیدگاه شما در مقاله {} پاسخی ثبت شده است برای مشاهده بر روی لینک زیر کلیک کنید\n{}{}.'.format(article,current_site,reverse('article:detail',kwargs={'slug':article.slug})),
                        to=[parent_email]
            )
            email.send()
